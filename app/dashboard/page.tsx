...
const styles = `
  ...
  *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
  ...
  :root {
    --bg:#f7f6f3; --white:#ffffff; --border:#e8e6e1; --border2:#f0ede8;
    --text:#1a1a18; --sub:#7a7870; --faint:#b8b5ae;
    --gold:#c8a96e; --gold-d:#a8894e; --gold-bg:rgba(200,169,110,.09); --gold-border:rgba(200,169,110,.22);
    --green:#2a7d4f; --green-bg:rgba(42,125,79,.08);
    --red:#c0392b; --red-bg:rgba(192,57,43,.07);
    --blue:#2563a8; --blue-bg:rgba(37,99,168,.08);
    --r:12px; --r-sm:8px;
  ...
  .sb-org-btn { display:flex; align-items:center; gap:8px; width:100%; padding:8px 12px; border-radius:var(--r-sm); background:var(--bg); border:1px solid var(--border2); cursor:pointer; margin-bottom:6px; transition:all .14s; text-align:left; }
  .sb-org-btn:hover { border-color:var(--gold-border); background:var(--gold-bg); }
  .sb-org-av { width:26px; height:26px; border-radius:6px; background:var(--gold-bg); border:1.5px solid var(--gold-border); display:flex; align-items:center; justify-content:center; font-family:'Syne',sans-serif; font-weight:700; font-size:10px; color:var(--gold-d); flex-shrink:0; }
  .sb-org-name { font-size:12px; font-weight:600; color:var(--text); flex:1; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
  .sb-org-label { font-size:10px; color:var(--faint); }
  .sb-org-chevron svg { width:12px; height:12px; color:var(--faint); }
  .org-dropdown { background:var(--white); border:1px solid var(--border); border-radius:var(--r-sm); overflow:hidden; margin-bottom:6px; box-shadow:0 4px 16px rgba(0,0,0,.06); }
  .org-opt { display:flex; align-items:center; gap:10px; width:100%; padding:10px 12px; border:none; background:none; cursor:pointer; transition:background .12s; text-align:left; }
  .org-opt:hover { background:var(--bg); }
  .org-opt.current { background:var(--gold-bg); }
  .org-opt-av { width:28px; height:28px; border-radius:7px; background:var(--gold-bg); border:1.5px solid var(--gold-border); display:flex; align-items:center; justify-content:center; font-family:'Syne',sans-serif; font-weight:700; font-size:10px; color:var(--gold-d); flex-shrink:0; }
  .org-opt-name { font-size:13px; font-weight:600; color:var(--text); flex:1; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
  .org-opt-check svg { width:13px; height:13px; color:var(--gold-d); }

  /* MOBILE TOP BAR */
  .mobile-topbar { display:none; align-items:center; justify-content:space-between; padding:14px 20px; background:var(--white); border-bottom:1px solid var(--border); position:sticky; top:0; z-index:40; }
  .mtb-logo { display:flex; align-items:center; gap:8px; }
  .mtb-mark { width:28px; height:28px; background:var(--text); border-radius:7px; display:flex; align-items:center; justify-content:center; }
  .mtb-mark svg { width:14px; height:14px; stroke:white; fill:none; }
  .mtb-name { font-family:'Syne',sans-serif; font-weight:700; font-size:15px; letter-spacing:-.4px; }
  .mtb-name span { color:var(--gold-d); }
  .mtb-right { display:flex; align-items:center; gap:8px; }
  .org-switch-btn { display:flex; align-items:center; gap:6px; padding:6px 11px; background:var(--bg); border:1px solid var(--border); border-radius:20px; font-family:'DM Sans',sans-serif; font-size:12px; font-weight:500; color:var(--sub); cursor:pointer; transition:all .15s; max-width:180px; }
  .org-switch-btn:hover { border-color:var(--gold-border); color:var(--gold-d); }
  .org-switch-btn .osb-av { width:20px; height:20px; border-radius:5px; background:var(--gold-bg); border:1px solid var(--gold-border); display:flex; align-items:center; justify-content:center; font-family:'Syne',sans-serif; font-weight:700; font-size:9px; color:var(--gold-d); flex-shrink:0; }
  .org-switch-btn .osb-label { white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:100px; }
  .org-switch-btn svg { width:12px; height:12px; flex-shrink:0; opacity:.5; }

  /* ORG DRAWER */
  .org-drawer { position:fixed; inset:0; z-index:200; }
  .org-drawer-bg { position:absolute; inset:0; background:rgba(0,0,0,.45); backdrop-filter:blur(2px); animation:fadeIn .18s ease; }
  .org-drawer-sheet { position:absolute; bottom:0; left:0; right:0; background:var(--white); border-radius:20px 20px 0 0; border-top:1px solid var(--border); padding:16px 20px max(env(safe-area-inset-bottom),28px); animation:slideUp .22s cubic-bezier(.34,1.2,.64,1); }
  @keyframes slideUp { from{transform:translateY(100%)} to{transform:translateY(0)} }
  .drawer-handle { width:36px; height:4px; background:var(--border); border-radius:2px; margin:0 auto 20px; }
  .drawer-title { font-family:'Syne',sans-serif; font-size:17px; font-weight:800; letter-spacing:-.5px; margin-bottom:4px; }
  .drawer-sub { font-size:13px; color:var(--sub); margin-bottom:18px; }
  .drawer-list { display:flex; flex-direction:column; gap:8px; margin-bottom:14px; }
  .drawer-item { display:flex; align-items:center; gap:12px; padding:14px 16px; border-radius:12px; border:1.5px solid var(--border); background:none; cursor:pointer; font-family:'DM Sans',sans-serif; text-align:left; width:100%; transition:all .15s; }
  .drawer-item:hover, .drawer-item.sel { border-color:var(--gold-border); background:var(--gold-bg); }
  .drawer-item .d-av { width:38px; height:38px; border-radius:10px; background:var(--gold-bg); border:1.5px solid var(--gold-border); display:flex; align-items:center; justify-content:center; font-family:'Syne',sans-serif; font-weight:700; font-size:13px; color:var(--gold-d); flex-shrink:0; }
  .drawer-item .d-body { flex:1; min-width:0; }
  .drawer-item .d-name { font-size:14px; font-weight:600; color:var(--text); }
  .drawer-item .d-id { font-size:11px; color:var(--faint); margin-top:2px; }
  .drawer-item .d-status { font-size:10px; font-weight:700; padding:3px 9px; border-radius:10px; text-transform:uppercase; letter-spacing:.3px; flex-shrink:0; }
  .drawer-item .d-status.active { background:var(--green-bg); color:var(--green); }
  .drawer-item .d-status.inactive { background:var(--red-bg); color:var(--red); }
  .drawer-item .d-check { width:20px; height:20px; border-radius:50%; border:2px solid var(--border); display:flex; align-items:center; justify-content:center; flex-shrink:0; }
  .drawer-item.sel .d-check { background:var(--gold); border-color:var(--gold); }
  .drawer-item.sel .d-check svg { display:block; }
  .drawer-item .d-check svg { display:none; width:10px; height:10px; stroke:#fff; stroke-width:3; fill:none; }
  .drawer-cancel { width:100%; padding:13px; border-radius:12px; border:1.5px solid var(--border); background:none; font-family:'DM Sans',sans-serif; font-size:14px; font-weight:500; color:var(--sub); cursor:pointer; transition:all .15s; }
  .drawer-cancel:hover { border-color:var(--text); color:var(--text); }
  ...
  /* BALANCE HERO */
  .balance-hero { background:var(--text); border-radius:var(--r); padding:26px 30px; box-shadow:var(--shadow-md); position:relative; overflow:hidden; animation:up .35s ease both; }
  ...
  /* QR CARD */
  .qr-card { background:var(--white); border:1px solid var(--border); border-radius:var(--r); padding:22px 24px; box-shadow:var(--shadow); display:flex; flex-direction:column; align-items:center; justify-content:center; gap:14px; text-align:center; }
  .qr-canvas-wrap { width:130px; height:130px; border-radius:10px; background:var(--white); padding:8px; border:1px solid var(--border2); display:flex; align-items:center; justify-content:center; flex-shrink:0; }
  .qr-mid-label { font-size:10px; font-weight:600; letter-spacing:1.5px; text-transform:uppercase; color:var(--faint); }
  .qr-mid-id { font-family:'Syne',sans-serif; font-size:13px; font-weight:700; letter-spacing:.5px; color:var(--text); }
  ...
  .a1{animation:up .38s .05s ease both} .a2{animation:up .38s .09s ease both}
  .a3{animation:up .38s .13s ease both} .a4{animation:up .38s .17s ease both}
  .a5{animation:up .38s .21s ease both} .a6{animation:up .38s .25s ease both}
  .a7{animation:up .38s .29s ease both} .a8{animation:up .38s .33s ease both}
  ...
  .bar-col:hover .bar-tooltip { opacity:1; transform:translateX(-50%) translateY(0); pointer-events:auto; }
  .bar-pill { width:100%; border-radius:5px 5px 0 0; transition:height .55s cubic-bezier(.34,1.3,.64,1),opacity .2s; cursor:default; }
  ...
  .bar-tooltip { position:absolute; bottom:calc(100% + 6px); left:50%; transform:translateX(-50%) translateY(4px); background:var(--text); color:#fff; border-radius:6px; padding:5px 9px; font-size:11px; font-weight:600; white-space:nowrap; opacity:0; transition:opacity .15s,transform .15s; pointer-events:none; z-index:10; }
  ...
  .modal-input { width:100%; border:1.5px solid var(--border); border-radius:var(--r-sm); padding:11px 14px; font-family:'DM Sans',sans-serif; font-size:16px; font-weight:600; color:var(--text); outline:none; transition:border-color .18s; margin-bottom:18px; background:var(--white); }
  .modal-input:focus { border-color:var(--gold); }
  .modal-acts { display:flex; gap:9px; }
  .btn-cancel { flex:1; padding:11px; border-radius:var(--r-sm); border:1.5px solid var(--border); background:none; font-family:'DM Sans',sans-serif; font-size:13px; font-weight:600; color:var(--sub); cursor:pointer; }
  .btn-cancel:hover { border-color:var(--text); color:var(--text); }
  .btn-confirm { flex:2; padding:11px; border-radius:var(--r-sm); border:none; background:var(--text); color:#fff; font-family:'DM Sans',sans-serif; font-size:13px; font-weight:600; cursor:pointer; }
  ...
  /* RESPONSIVE */
  @media(max-width:1080px){ .layout{grid-template-columns:200px 1fr} .main{padding:32px 28px} .row3{grid-template-columns:1fr 1fr} }
  @media(max-width:820px){
    .layout{grid-template-columns:1fr} .sidebar{display:none}
    .mobile-topbar{display:flex} .mobile-nav{display:block}
    .main{padding:22px 18px 90px}
    .row1{grid-template-columns:1fr 1fr} .row2{grid-template-columns:1fr} .row3{grid-template-columns:1fr}
    .bh-amount{font-size:32px} .balance-hero{padding:20px 22px}
    .bh-footer{flex-direction:column;align-items:flex-start;gap:12px}
  }
  @media(max-width:480px){ .row1{grid-template-columns:1fr} .amts{grid-template-columns:repeat(2,1fr)} .tb-name{font-size:22px} }
`;
...
const orgInits = n => (n||"?").split(" ").slice(0,2).map(w=>w[0]?.toUpperCase()).join("").slice(0,2);
...
const sniffCurrency = (txs=[]) => { for(const t of txs){const m=(t.description||"").match(/([A-Z]{2,4})\d/);if(m)return m[1];}return "XAF"; };
...
const buildWeekly = (txs=[]) => {
  ...
  return labels.map((lbl,i) => {
    ...
    const dayTxs = txs.filter(t=>(t.transaction_date||t.created_at||t.date||"").slice(0,10)===ds);
    const spend = dayTxs.filter(t=>isDebit(t.transaction_type)).reduce((s,t)=>s+Math.abs(t.amount||0),0);
    return { lbl, spend, isToday:i===todayIdx, isFuture:i>todayIdx };
  });
};
...
export default function Dashboard() {
  ...
  const allMemberships = typeof window!=="undefined"
    ? (JSON.parse(localStorage.getItem("ms_memberships")||"null") || (stored?[stored]:[]))
    : [];
  const multiOrg = allMemberships.length > 1;
  ...
  useEffect(()=>{
    ...
    fetch(`${API_BASE}/members/${mid}/prepaid`).then(r=>r.json()).then(j=>{if(j.success)setPrepaid(j);}).catch(()=>{}).finally(()=>setLW(false));
  ...
  const currentIn = !!(checkins||[])[0] && (checkins||[])[0]?.status==="checked_in" && !(checkins||[])[0]?.checkout_time;

  const nav = href => () => window.location.href = href;
  const switchOrg = m => { localStorage.setItem("ms_member",JSON.stringify(m)); window.location.reload(); };
  const Sk = ({w="100%",h=14,r=6,...p}) => <span className="sk" style={{width:w,height:h,borderRadius:r,...p}}/>;

  const QRCanvas = ({ value, size=110 }) => {
    const [imgLoaded, setImgLoaded] = useState(false);
    const [err, setErr] = useState(false);
    const src = value ? `https://api.qrserver.com/v1/create-qr-code/?size=${size*2}x${size*2}&data=${encodeURIComponent(value)}&color=1a1a18&bgcolor=ffffff&qzone=1&format=png` : null;
    if (!src) return <Sk w={size} h={size} r={8}/>;
    return (
      <div style={{position:"relative",width:size,height:size}}>
        {!imgLoaded&&!err&&<Sk w={size} h={size} r={8}/>}
        {err&&<div style={{width:size,height:size,display:"flex",alignItems:"center",justifyContent:"center",background:"var(--bg)",borderRadius:8,fontSize:10,color:"var(--faint)",textAlign:"center",padding:8}}>QR unavailable</div>}
        <img src={src} width={size} height={size} alt={`QR:${value}`}
          style={{display:imgLoaded?"block":"none",borderRadius:4,imageRendering:"crisp-edges"}}
          onLoad={()=>setImgLoaded(true)} onError={()=>setErr(true)}/>
      </div>
    );
  };
    ...
    <>
      ...
      <div className="layout">
        ...
        <div className="mobile-topbar">
          <div className="mtb-logo">
            ...
            <span className="mtb-name">Member<span>Sync</span></span>
          </div>
          <div className="mtb-right">
            {multiOrg && (
              <button className="org-switch-btn" onClick={()=>setOrgDrawer(true)}>
                <div className="osb-av">{orgInits(stored?.organization_name)}</div>
                <span className="osb-label">{stored?.organization_name||"My Org"}</span>
        ...
        <aside className="sidebar">
          ...
          <div className="sb-footer">
            {multiOrg && (
              <div style={{marginBottom:6}}>
                <button className="sb-org-btn" onClick={()=>setShowOrgDrop(v=>!v)}>
                  <div className="sb-org-av">{orgInits(stored?.organization_name||"")}</div>
                ...
                {showOrgDrop && (
                  <div className="org-dropdown">
                    {allMemberships.map(m=>(
                      <button key={m.membership_id} className={`org-opt${m.membership_id===mid?" current":""}`}
                        onClick={()=>{ setShowOrgDrop(false); if(m.membership_id!==mid) switchOrg(m); }}>
                        <div className="org-opt-av">{orgInits(m.organization_name||"")}</div>
                        <div style={{flex:1,minWidth:0}}>
                        ...
                        {m.membership_id===mid&&(
                          <div className="org-opt-check">
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M20 6 9 17l-5-5"/></svg>
                          </div>
                        )}
                      </button>
                    ))}
            ...
            <button className="sb-signout" onClick={()=>{ localStorage.removeItem("ms_member"); localStorage.removeItem("ms_memberships"); window.location.href="/"; }}>
              <ILogout/>Sign Out
            </button>
            <div className="sb-member">
              <div className="sb-av">{profile?.photo_url?<img src={profile.photo_url} alt={name}/>:initials(name)}</div>
        ...
        <main className="main">
            ...
            <div className="topbar-right">
              {!lP&&profile?.status&&(
                <div className={`chip ${profile.status==="active"?"active":"inactive"}`}>
                  <div className="sdot"/>{profile.status==="active"?"Active":"Inactive"}
                </div>
              )}
              {currentIn&&<div className="chip inside"><ILogIn style={{width:12,height:12}}/>Inside</div>}
              <div className="tb-av">
                {lP?<Sk w={40} h={40} r={20}/>:profile?.photo_url?<img src={profile.photo_url} alt={name}/>:initials(name)}
              </div>
            </div>
          </div>

          {/* ROW 1: Balance hero + QR */}
            ...
            <div style={{gridColumn:"span 2"}}>
              <div className="balance-hero">
                ...
                {lW?<Sk w={200} h={40} r={8}/>:(
                  <div className="bh-amount"><span className="bh-cur">{currency}</span>{fmtAmt(balance)}</div>
                )}
                <div className="bh-footer">
                  ...
                  <button className="btn-topup" onClick={()=>setShowModal(true)}><IPlus/>Top Up</button>
                </div>
              </div>
            </div>
            <div className="qr-card a1">
              {lP?<Sk w={120} h={120} r={10}/>:(
                <>
                  <div className="qr-canvas-wrap"><QRCanvas value={mid} size={110}/></div>
                  <div><div className="qr-mid-label">Member ID</div><div className="qr-mid-id">{mid}</div></div>
                  <div className="qr-visits-row">
                    <IHistory/>
                    {lC?<Sk w={20} h={12} r={3}/>:<span className="qr-visits-num">{(checkins||[]).length}</span>}
          ...
          <div className="row2" style={{marginBottom:14}}>
            <div className="card a3">
              <div className="sh"><div className="sh-title">Membership</div><button className="sh-action" onClick={nav("/membership")}>Renew →</button></div>
              <div className="ring-card">
                <div className="ring-wrap">
                  <svg className="ring-svg" viewBox="0 0 100 100">
                    <circle className="ring-track" cx="50" cy="50" r={R}/>
                    <circle className={`ring-fill ${lP?"good":expCls}`} cx="50" cy="50" r={R} strokeDasharray={CIRC} strokeDashoffset={lP?CIRC:ringOffset(dl)}/>
                  </svg>
                  <div className="ring-center">
                    {lP?<Sk w={28} h={18} r={4}/>:<><div className="ring-num">{!dl||dl<=0?"0":dl>365?"365+":dl}</div><div className="ring-unit">days</div></>}
                  </div>
                </div>
                <div>
                  {lP?<><Sk w={110} h={17} r={5}/><Sk w={80} h={12} r={4} style={{marginTop:8}}/></>:<>
                    ...
                    <div className={`ring-badge ${expCls}`}>{expCls==="good"?"Active":expCls==="warning"?"Expiring soon":"Expired"}</div>
                  </>}
                </div>
              </div>
            </div>
            ...
            <div className="card a4">
              <div className="sh">
                ...
                <div className="chart-tabs"><button className="ctab on">This week</button></div>
              </div>
              {lW?(
                <div className="chart-body"><div className="bars-row">{[55,40,75,60,85,35,70].map((h,i)=><div key={i} className="bar-col"><Sk w="100%" h={`${h}%`} r={5}/></div>)}</div></div>
              ):(()=>{
                const total = weekly.reduce((s,d)=>s+d.spend,0);
                return (
                  <div className="chart-body">
                    <div className="bars-row">
                      {weekly.map((d,i)=>{
                        const hp = maxBar>0?Math.max(0,(d.spend/maxBar)*100):0;
                        const cls = d.isToday?"today":d.isFuture?"future":"past";
                        return (
                          <div key={i} className="bar-col">
                            {d.spend>0&&<div className="bar-tooltip">{currency} {fmtAmt(d.spend)}</div>}
                            <div className={`bar-pill ${cls}`} style={{height:hp>0?`${hp}%`:(d.isFuture?"6px":"3px")}}/>
                            <div className={`bar-day${d.isToday?" today":d.isFuture?" future":""}`}>{d.lbl}</div>
                          </div>
                        );
                      })}
                    </div>
                    <div className="chart-total-row">
                      ...
                      <span className="chart-total-val">{total>0?`${currency} ${fmtAmt(total)}`:"No spending this week"}</span>
                    </div>
                  </div>
                );
              })()}
          ...
          {/* ROW 3: Services + Check-ins + Notifs */}
          <div className="row3">
            <div className="card a5">
              <div className="sh"><div className="sh-title">Most Visited</div><button className="sh-action" onClick={nav("/checkin")}>History →</button></div>
              {lC?(
              ...
              ):services.length===0?<div className="empty">No visit data yet.</div>:(
                <div className="svc-list">
                  {services.map(([svc,cnt],i)=>(
                    <div className="svc-row" key={svc}>
                      <div className="svc-rank">{i+1}</div>
                      <div className="svc-body">
                        <div className="svc-top"><span className="svc-name">{svc}</span><span className="svc-cnt">{cnt}×</span></div>
                        <div className="svc-track"><div className="svc-fill" style={{width:`${(cnt/maxSvc)*100}%`}}/></div>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="card a6">
              <div className="sh"><div className="sh-title">Recent Check-ins</div><button className="sh-action" onClick={nav("/checkin")}>All →</button></div>
              {lC?(
              ...
              ):(checkins||[]).length===0?<div className="empty">No visits yet.</div>:(
                  ...
                  {(checkins||[]).slice(0,4).map((h,i)=>{
                    ...
                    return (
                      <div className="ci-row" key={i}>
                        ...
                        <div className="ci-info"><div className="ci-svc">{h.service_type?.trim()||"Facility"}</div><div className="ci-time">{fmtDT(h.checkin_time)}</div></div>
            ...
            <div className="card a7">
              <div className="sh">
                <div className="sh-title">Alerts{unread>0&&<span style={{marginLeft:7,background:"var(--gold)",color:"#fff",borderRadius:20,padding:"1px 7px",fontSize:10,fontWeight:700}}>{unread}</span>}</div>
              ...
              {lN?(
                ...
                <div className="notif-list">
                  {(notifs||[]).slice(0,4).map((n,i)=>(
                    <div key={i} className={`notif-row${n.status==="read"?" read":""}`} onClick={nav("/notifications")}>
                      <div className={`n-dot ${n.status==="read"?"r":"u"}`}/>
                      <div style={{flex:1,minWidth:0}}>
                        <div className="n-msg">{n.message}</div>
                        <div className="n-meta">{fmtDT(n.sent_at||n.created_at)}</div>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      {orgDrawer&&(
        <div className="org-drawer">
          <div className="org-drawer-bg" onClick={()=>setOrgDrawer(false)}/>
          <div className="org-drawer-sheet">
            <div className="drawer-handle"/>
            <div className="drawer-title">Switch Membership</div>
            <div className="drawer-sub">You have {allMemberships.length} memberships</div>
            <div className="drawer-list">
              {allMemberships.map(m=>(
                <button key={m.membership_id} className={`drawer-item${m.membership_id===mid?" sel":""}`}
                  onClick={()=>{ setOrgDrawer(false); if(m.membership_id!==mid) switchOrg(m); }}>
                  <div className="d-av">{orgInits(m.organization_name||"")}</div>
                  <div className="d-body">
                    <div className="d-name">{m.organization_name||"Organization"}</div>
                    <div className="d-id">{m.membership_id}</div>
                  </div>
                  <div className={`d-status ${m.status==="active"?"active":"inactive"}`}>{m.status==="active"?"Active":"Inactive"}</div>
                  <div className="d-check"><svg viewBox="0 0 24 24"><path d="M20 6 9 17l-5-5"/></svg></div>
                </button>
              ))}
            </div>
            <button className="drawer-cancel" onClick={()=>setOrgDrawer(false)}>Cancel</button>
          </div>
        </div>
      )}
